/* EX LIBRIS JURIS v5.76 — api/lib/style_guide.js
   ...
   text with the amendments agreed on 24 Sep 2026:
     - the opening says section 4 (Accuracy) is never overridden by a tool's
       own instructions, which the original contradicted;
     ...
     - v5.76: briefing notes and issue briefings are candid, not advocacy.

   Two texts are exported:
     STYLE_GUIDE_FULL      draft, briefing, issue briefing
     STYLE_GUIDE_ANALYSIS  analysis tools and chat: accuracy and restraint
                           only, because the voice, numbering and flag-list
                           rules do not fit a chronology table or an issue
                           tracker.

   Both go into the SYSTEM prompt. They must stay byte-identical from call to
   call: nothing per-job belongs in here, or the prompt cache breaks (see
   CLAUDE.md, "Prompt caching").

   Feature flag: ELJ_STYLE_GUIDE=off in the Vercel environment makes
   styleGuideFor() return "" everywhere, restoring the pre-v5.75 prompts
