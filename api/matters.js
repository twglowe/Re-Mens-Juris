...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      // Shared matters
      const { data: shares } = await supabase.from("matter_shares")
        .select("matter_id, permission, matters(*)")
        .eq("user_id", user.id);
    ...
    if (req.method === "POST") {
      ...
      const { data, error } = await supabase.from("matters")
        .insert({ name, jurisdiction: jurisdiction || "Bermuda", owner_id: user.id, document_count: 0 })
        .select().single();
